function [tform, movingReg, rmse, iteration, maxOutlier, minOutlier, maxKeepInlier, minKeepInlier, normmaxInlierDist, normminInlierDist] = mypcregrigid(moving, fixed, varargin)
...
for i = 1 : maxIterations
    ...
    if(numel(inlierIndicesA)>maxOutlier)
        maxOutlier = numel(inlierIndicesA);
    end
    
    if(numel(inlierIndicesA)<minOutlier)
        minOutlier = numel(inlierIndicesA);
    end
    inlierIndicesB = indices(keepInlierA);
    inlierDist = dists(keepInlierA);
    if(inlierDist>maxInlierDist)
        maxInlierDist = inlierDist;
    end
    
    if(inlierDist<minInlierDist)
        minInlierDist = inlierDist;
    end
    if(numel(keepInlierA)>maxKeepInlier)
        maxKeepInlier = numel(keepInlierA);
    end
    
    if(numel(keepInlierA)<minKeepInlier)
        minKeepInlier = numel(keepInlierA);
    end
...
normminInlierDist = norm(minInlierDist);
normmaxInlierDist = norm(maxInlierDist);
iteration = i;
